...
- Finally renamed to HerokuX and new Emoji
...
- added config for show "HerokuX" in info
- removed UnitHeta
...
- Completely drop HerokuX support due to legacy code, limits and removing of free tier
...
- Publish hikka telethon fork and migrate to it in requirements, thereby fixing the deployment error on HerokuX
...
- Add support for multiaccounting on HerokuX
...
- Add `websockets` dependency, so users can load hikarichat on HerokuX
...
- New HerokuX dependencies list in Pipfile
...
- Fix `Unauthorized` error on HerokuX
...
- Add full-featured HerokuX support with additional buildpacks
