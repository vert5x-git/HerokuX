...
class UpdateNotifier(loader.Module):
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            m = await self.inline.bot.send_animation(
                ...
                caption=self.strings("update_required").format(
                    ...
                    '<a href="https://github.com/coddrago/HerokuX/compare/{}...{}">{}</a>'.format(
    ...
    async def changelog(self, message: Message):
        ...
        if (await self._client.get_me()).premium:
            changelog.replace('🌑 HerokuX', '<emoji document_id=5192765204898783881>🌘</emoji><emoji document_id=5195311729663286630>🌘</emoji><emoji document_id=5195045669324201904>🌘</emoji>')
