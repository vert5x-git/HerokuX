...
# Клонируем репозиторий HerokuX
RUN git clone https://github.com/coddrago/HerokuX /HerokuX
# Создаем виртуальное окружение Python
RUN python -m venv /venv
# Устанавливаем зависимости проекта
RUN /venv/bin/pip install --no-warn-script-location --no-cache-dir -r /HerokuX/requirements.txt
...
COPY --from=builder /HerokuX /HerokuX
COPY --from=builder /venv /HerokuX/venv
